...
void setup()
{
    ...
    CAN.init_Mask(1, STD, 0x0);
        ...
        WiFi.softAPConfig(IPAddress(192, 168, 1, 1),
                          ...
                          IPAddress(0, 0, 0, 0), // Чтобы комп или телефон не пытался выходить в инет через наш ESP, лучше сказу передать невалидный шлюз.
...
void loop()
{
	...
	// Большинство плат не имеют метода `serialEvent()`, поэтому полагаться на него не стоит.
    #ifdef COM
        MySerialEvent();
    #endif
}
